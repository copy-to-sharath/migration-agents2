#!/usr/bin/env python3
"""
Generate DOT graphs by depth level with separate entry and exit node identification.

For each depth level, generates a graph showing:
- Entry nodes (in-degree = 0 within that depth's subgraph) - colored orange
- Exit nodes (out-degree = 0 within that depth's subgraph) - colored red
- Regular nodes - colored blue (current depth) or green (next depth)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Generate call graphs by depth level with entry/exit identification'
    )
    parser.add_argument(
        '--run-id',
        type=str,
        help='Run ID to use (e.g., run_20260122130739). Defaults to latest.'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='generated/graphs/depth_entry_exit',
...
def compute_depths(G: nx.DiGraph) -> Dict[str, int]:
    """Compute depth for each node from entry points."""
    ...
    # Mark unreachable nodes (cycles)
    unreachable = set(G.nodes()) - set(node_depths.keys())
...
def generate_dot(
    ...
    """Generate DOT file for a depth level with entry/exit highlighting."""
    with open(output_path, 'w') as f:
        f.write(f'digraph CallGraph_Depth_{depth} {{\n')
        # Compact graph settings for single-page viewing
        f.write('  rankdir=LR;\n')
        ...
        f.write(f'  label="D{depth} E:{len(entries)} X:{len(exits)}";\n')
        f.write('  labelloc=t;\n\n')
        
        # Compact node and edge styles
        f.write('  node [fontname="Helvetica", fontsize=8, shape=box, style="filled,rounded", penwidth=1, margin="0.05,0.02"];\n')
        f.write('  edge [fontname="Helvetica", fontsize=7, color="#666666", penwidth=1, arrowsize=0.5];\n\n')
        
        # Compact legend
        f.write('  subgraph cluster_legend {\n')
        f.write('    label="";\n')
        f.write('    style=invis;\n')
        f.write('    legend_entry [label="E", fillcolor="#ff9500", fontcolor="white", width=0.3, height=0.2];\n')
        f.write('    legend_exit [label="X", fillcolor="#ff6b6b", fontcolor="white", width=0.3, height=0.2];\n')
        f.write('    legend_both [label="E+X", fillcolor="#ffd93d", width=0.3, height=0.2];\n')
        f.write('  }\n\n')
        ...
        for node_id in subgraph.nodes():
            ...
            if len(label_text) == 64 and label_text.isalnum():
                ...
                short_label = escape_label(label_text[:30])
            ...
            if node_id in entries and node_id in exits:
                fillcolor = '#ffd93d'
                fontcolor = 'black'
            elif node_id in entries:
                fillcolor = '#ff9500'
                fontcolor = 'white'
            elif node_id in exits:
                fillcolor = '#ff6b6b'
                fontcolor = 'white'
...
def main() -> None:
    ...
    # Load nodes from code_graph_nodes
    nodes = con.execute(f"""
    ...
    # Load edges from code_graph_edges
    edges = con.execute(f"""
    ...
    global_entries = [n for n in G.nodes() if G.in_degree(n) == 0]
    print(f'Entry points: {len(global_entries):,}')
    ...
    # Compute depths from entry points
    print('\nComputing depths from entry points...')
    node_depths = compute_depths(G)
    
    global_exits = [n for n in G.nodes() if G.out_degree(n) == 0]
    print(f'Exit points: {len(global_exits):,}')
    ...
    for depth in sorted(depth_groups.keys()):
        entries, exits = find_entries_exits_at_depth(G, node_depths, depth)
        print(f'  Depth {depth}: {len(depth_groups[depth]):,} nodes, {len(entries)} entries, {len(exits)} exits')
    
    # Create output directory
    output_dir = args.output_dir
    os.makedirs(output_dir, exist_ok=True)
    
    print(f'\nGenerating graphs with entry/exit nodes to {output_dir}...')
    ...
    for depth in sorted(depth_groups.keys()):
        ...
        generate_dot(subgraph, depth, node_info, node_depths, pagerank_scores, entries, exits, dot_file)
        
        print(f'  Depth {depth}: {len(nodes_at_depth):,} nodes, {len(entries)} entries, {len(exits)} exits → {dot_file}')
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f'Entry points: {len(global_entries):,}\n')
        f.write(f'Exit points: {len(global_exits):,}\n\n')
        f.write('Depth distribution:\n')
        for depth in sorted(depth_groups.keys()):
            entries, exits = find_entries_exits_at_depth(G, node_depths, depth)
            f.write(f'  Depth {depth}: {len(depth_groups[depth]):,} nodes, {len(entries)} entries, {len(exits)} exits\n')
